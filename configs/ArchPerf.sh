...
( bash $SCRIPT_DIR/scripts/ask.sh )|& tee ask.log
    ...
    if [[ ! $DESKTOP_ENV == server ]]; then
      ( arch-chroot /mnt /usr/bin/runuser -u $USERNAME -- /home/$USERNAME/ArchPerf/scripts/2-user.sh )|& tee 2-user.log
    fi
    ( arch-chroot /mnt $HOME/ArchPerf/scripts/3-post-setup.sh )|& tee 3-post-setup.log
