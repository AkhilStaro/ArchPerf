...
USER_PREF=$CONFIGS_DIR/user_pref.conf
if [ ! -f $USER_PREF ]; then # check for file
    touch -f $USER_PREF # create file if not exists
fi

# Set Options In The user_pref.conf File
set_option() {
    if grep -Eq "^${1}.*" $USER_PREF; then # check if option exists
        sed -i -e "/^${1}.*/d" $USER_PREF # delete option if exists
    fi
    echo "${1}=${2}" >>$USER_PREF # add option
}
